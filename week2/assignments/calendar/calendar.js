...
for (var i = 0; i < months.length; i++) {
   ...
   // console.log(currentDate.getMonth());
    if (i == currentDate.getMonth())
        isSelected = true;
    allM.append(new Option(months[i], i, false, isSelected ));
 }
...
function printCalendar(){
    var allM = document.querySelector("#allM").value;
    var allY = document.querySelector("#allY").value;

        

    //const today = new Date();
    const today = new Date(allY, allM, 1);
    //console.log(today)
...
while ( y < remainder) {
  ...
  console.log(allM, allY);
 }
    
    document.querySelector("#allY").value = currentDate.getUTCFullYear();
    printCalendar();
